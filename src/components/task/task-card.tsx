    ...
    <div className="flex flex-col gap-3 rounded-card bg-surface p-4 shadow-card">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-ink-muted">{categoryLabel(task.category)}</p>
        </div>
        <StatusBadge status={task.status} />
      </div>

      <p className="line-clamp-2 text-sm text-ink-soft">{task.description}</p>
      ...
      <div className="flex items-center justify-between border-t border-line pt-3">
        <div>
          <span className="text-xs text-ink-muted">{feeLabel}</span>
          <p className="font-bold text-primary-dark">{formatRupiah(fee)}</p>
        </div>
        {footer}
      </div>
    </div>
